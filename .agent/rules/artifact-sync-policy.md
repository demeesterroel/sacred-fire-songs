...
| `master-tasks.md` | **OVERWRITE** | **Caution**. Agent **MUST READ** the existing file first to merge history with new tasks. |
| `master-timetracking.md` | **OVERWRITE** | **Caution**. Agent **MUST READ** the existing file first to update totals. |
...
3.  **Safety Check**: Never overwrite `master-tasks.md` or `master-timetracking.md` with an empty or partial list. ensuring previously completed items are preserved.
