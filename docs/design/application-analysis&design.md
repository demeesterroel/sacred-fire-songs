...
| **1.32** | July 16, 2026 | Moved existing song folders from `doc/extracted_songs` to `data/extracted_songs` and created a Python sync script (`sync_prod_songs.py`) to extract and sync all Production DB songs locally to `.cho` files ignored by Git. |
...
* **Isolation Strategy:** Playwright E2E tests are configured to run against a separate, online staging Supabase instance (`wuigxbpwkpjqqiystbyz`). This preserves the integrity of the self-hosted Supabase database used to drive the `songbook-beta` staging application.
...
* **Programmatic Random Seeding:** When the environment variable `E2E_RANDOM_SEED=1` is set, `setup-test-db.mjs` triggers a dynamic seeder (`scripts/random-seeder.mjs`) that generates rich, randomized song titles, ChordPro content, media links, elements/lineage taxonomy mappings, and private user rehearsal recordings (staggered takes for 30% of songs, and multiple 2-5 takes for 50% of songs). This exercises the system's filtering, layout, search, and private recording drawer list views.
