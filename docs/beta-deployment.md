...
**https://songbook-beta.bluette.be**.
...
| --- | --- | --- | --- |
| URL | *(Vercel prod domain)* | `sacred-fire-songs-*.vercel.app` | `songbook-beta.bluette.be` |
...
| Supabase | **prod** project | **staging** project (`wuigxbpwkpjqqiystbyz`) | **staging** project (`wuigxbpwkpjqqiystbyz`) |
...
3. Is published behind Traefik at `songbook-beta.bluette.be`.
...
4. **Verify:** open `https://songbook-beta.bluette.be` and confirm your change is live.
...
- [ ] Supabase **auth redirect URLs** for the staging project must include
      `https://songbook-beta.bluette.be/**`, or magic-link / OTP / password-reset
      logins fail with `redirect_to is not allowed`.
- [ ] Staging `storage` bucket may be empty (avatars won't render) until recreated.
- [ ] DNS for `songbook-beta.bluette.be` is covered by a `*.bluette.be` wildcard — no
