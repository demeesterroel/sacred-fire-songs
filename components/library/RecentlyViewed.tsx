      ...
      <div className="grid grid-cols-1 gap-2">
        {songs.map((song) => (
          <Link
            key={song.id}
            href={`/songs/${song.id}`}
            className="flex items-center gap-3 p-3 rounded-xl bg-white/60 dark:bg-gray-900/30 border border-gray-200/50 dark:border-gray-800/50 hover:bg-gray-200/50 dark:hover:bg-gray-800/50 hover:border-gray-300/50 dark:hover:border-gray-700/50 transition-all group"
          >
            ...
            <div className="min-w-0 flex-1">
              <p className="text-sm font-semibold text-gray-800 dark:text-gray-200 truncate group-hover:text-gray-900 dark:group-hover:text-white transition-colors">{song.title}</p>
              {song.author && (
